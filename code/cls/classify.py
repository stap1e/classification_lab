...
def main():
    data_train_path = './new_data/0714data_delete.xlsx' 
    time_lab = data_train_path.split('data')[1].split('/')[-1]
    # nse_path = 't1_nse_new_sorted.xlsx'
    # nse_mode = nse_path.split('_')[0]
    ct_mode = data_train_path.split('data')[0].split('/')[-1]
    # try:
    #     nse_mode == ct_mode
    # except Exception as e:
    #     print(f"nse_time and ct_time is not same: {str(e)}")
        
    # read features
    # ct_features = pd.read_excel(data_train_path)
    data_train = pd.read_excel(data_train_path)
    # nse_features = pd.read_excel(nse_path)
    features = data_train.columns.drop(['CPC']).to_list()
    # data_train = ct_features.drop(['CTid'], axis=1)
    ...
    result_folder = get_next_result_folder(base_path='./results_0714_delete')
    ...
    catboost_params = {
        'iterations': 500,  # 迭代次数
        'depth': 6,  # 树的深度
        # The above code is defining a dictionary in Python with a key 'learning_rate' and a value of
        # 0.016. The comment after the value indicates that this value represents the learning rate.
    ...
    # y_pred_final = []
    # y_prob_final = []
    save_path = './results_{}/roc_curve_{}_time_{}'.format(time_lab, mode, ct_mode)
    ...
    for fold, (train_index, test_index) in enumerate(kf.split(X_index, y_index), 1):
        ...
        # ======================================================== 合并 nse 的特征 ========================================================
        # nse_max = []
        # nse_max = nse_features['MAX']
        # nse_range = nse_features['|MAX-MIN|']
        # nse_max_train = nse_max.iloc[train_index]
        # nse_range_train = nse_range.iloc[train_index]
        # nse_max_test = nse_max.iloc[test_index]
        # nse_range_test = nse_range.iloc[test_index]
        # data_train_label = data_train_lasso['label']
        # data_train_lasso['max'] = nse_max_train
        # data_train_lasso['range'] = nse_range_train
        # data_train_lasso = data_train_lasso.drop(['label'], axis=1)
        # data_train_lasso['label'] = data_train_label

        # if len(nse_max) > 0 :
        #     nse_mode_train = 'with nse'
        #     print(f"this is with nse train.")
        # else:
        #     nse_mode_train = 'without nse'
        print(f"data for {fold} 折 selected features num is : {len(selected_features)}")
        ...
        # ============================================= whether with nse step for validation =============================================
        # X_val['max'] = nse_max_test
        # X_val['range'] = nse_range_test
        ...
        selected_features_all.append(f"折数: {fold}, number: {len(selected_features)}, features: {selected_features}\n")
    ...
    results += "label 0 num: {}, label 1 num: {}\n".format(label0_num, label1_num) 
    # results += "whether with nse: {}\n".format(nse_mode_train)
    for metric, value in final_results.items():
        results += f"\n{metric}: {value}\n"
    for feature in selected_features_all:
        results += f"{feature}\n"
    results += "selected features:{}\n".format(selected_features_all)
